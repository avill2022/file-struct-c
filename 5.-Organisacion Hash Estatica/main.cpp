...
void iniciaTabla(char nombre[]);
void dibujaTablas(char nombre[]);
void guardallave(char nombre[],int llave,long dir_arch);
void eliminallave(char nombre[],int llave);
    ...
    do
    {
       ...
       if(arch.Archivo!=NULL)
       {
          ...
          if(arch.prim!=NULL && arch.ult!=NULL)
          {
               ...
               rectangle(72+140,2,140+140,28); 
               outtextxy(78+140,7,"Tablas");              
          }
       }
       ...
       if(ismouseclick(WM_LBUTTONDOWN))
       {
           ...
           if(x> 72+140 && y>2 && x< 140+140 && y<28)
           {
                printf("Menu tabla\n");
                Entidad *nuevo = seleccionaEntidad(arch);
                 if(nuevo!=NULL)
                 {
                      dibujaTablas(nuevo->datEnti.nombre);
                 }
                 cleardevice();                                                   
           }                           
       }
...
{ 
     ...
     iniciaTabla(ent->datEnti.nombre);
     cleardevice();
     if(arch->prim == NULL && arch->ult == NULL)
     {
         arch->Archivo = fopen("Archivo.bet","rb+"); 
         ...
         {   
             ...
             { 
                 ...
                 {                      
                       ...
                       {
                           guardallave(enti->datEnti.nombre,entero,reg->dir_reg);                
...
{   
     ...
     Registro *reg = ent->rp;
     Atributo *atri = ent->prim; 
     eliminallave(ent->datEnti.nombre,llaveEntero);
...
void iniciaTabla(char nombre[])
{
     ...
     for(int i=0;i<=N;i++)
     {
         ...
         for(int j =0;j<=N;j++)
         {
              tabla[i].hash[j].llave =0;  
              fwrite(&tabla[i].hash[j].llave,sizeof(int),1,nuevo); 
              tabla[i].hash[j].dir_arch = -1; 
              fwrite(& tabla[i].hash[j].dir_arch,sizeof(long),1,nuevo);
         }        
     }
     fclose(nuevo);
}
     ...
     while(!feof(nuevo))
     {
          ...
          if(mod == modulo)
          {
               ...
               for(int j =0;j<=N;j++)
               {
                   ...
                   if(mod == llave)
                   {
                          fseek(nuevo,pos,SEEK_SET);
                          fwrite(&reinicia,sizeof(int),1,nuevo);    
                          fwrite(&dirarch,sizeof(long),1,nuevo);    
                          printf("DatoEliminado correctamente\n");
                          fclose(nuevo);
                          return;
                   }
                   else
                   {
                       fread(&dat,sizeof(long),1,nuevo);     
                   } 
               } 
               fclose(nuevo);
               printf("cajon sin nada\n");
               return;
          }
          else
          {
     ...
     while(!feof(nuevo))
     {
          ...
          if(mod == modulo)
          {
               for(int j =0;j<=N;j++)
               {
                   ...
                   {
                          fseek(nuevo,pos,SEEK_SET);
                          fwrite(&llave,sizeof(int),1,nuevo);    
                          fwrite(&dir_arch,sizeof(long),1,nuevo);    
                          printf("Dato guardado correctamente\n");
                          fclose(nuevo);
                          return;
                   }
                   else
...
void dibujaTablas(char nombre[])
{
     ...
     for(int j=0;j<=3;j++)
     {
         rectangle(10,9+130*j,27,27+130*j);
         fread(&llave,sizeof(int),1,nuevo);
         itoa(llave,dato,10);
         outtextxy(14,10+130*j,dato);
         printf("%d\n",llave);
         printf("-------------------\n");
         for(int i =0;i<=3;i++)
         {
             rectangle(10,27+20*i+130*j,10+200,27+20*(i+1)+130*j);
             line(110,27+20*i+130*j,110,27+20*(i+1)+130*j);
             fread(&llave,sizeof(int),1,nuevo);
             itoa(llave,dato,10);
             printf("%d-",llave);
             outtextxy(14,27+20*i+3+130*j,dato);
             fread(&dir_arch,sizeof(long),1,nuevo);
             printf("%d\n",dir_arch);
             itoa(dir_arch,dato,10);
             outtextxy(14+150,27+20*i+3+130*j,dato);
         }
         printf("-------------------\n");
     }
     getch();
